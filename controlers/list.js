...
export const createList = async (req, res) => {
    ...
    const newList = new List({ ...room, creator: req.userId, updatedAt: new Date().toISOString() })
...
export const updateList = async (req, res) => {
    ...
    if (!mongoose.Types.ObjectId.isValid(id)) return res.status(404).send(`No room with id: ${id}`);
        ...
        const updatedList = {name,price,category,roomStatus,image, _id: id, updatedAt: new Date().toISOString()};
